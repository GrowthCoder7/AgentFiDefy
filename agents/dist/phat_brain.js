...
// src/phat_brain.ts
function main(input) {
  const provider = new ethers_exports.JsonRpcProvider("https://rpc.sepolia.org");
  const seed = pink.deriveSecret("sepolia-key");
  const wallet = new ethers_exports.Wallet(ethers_exports.hexlify(seed), provider);
  const response = pink.httpRequest({
    ...
    body: JSON.stringify({
      ...
      id: 1,
      method: "eth_getLogs",
      params: [{ address: "0xYourIncoStore", fromBlock: "latest" }]
    })
  });
  const bodyText = typeof response.body === "string" ? response.body : new TextDecoder().decode(response.body);
  const data = JSON.parse(bodyText);
  if (data.result && data.result.length >= 3) {
    return `Ready to sign for: ${wallet.address}`;
  }
  return "Waiting...";
}
export {
